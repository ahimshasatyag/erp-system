...
const ProductUnitTable: React.FC<ProductUnitTableProps> = ({
    ...
    const total = meta?.total ? parseInt(meta.total.toString(), 10) : units.length;
    ...
    return (
        <div className="w-full">
            ...
            <div className="overflow-x-auto border-t border-b border-gray-200 mt-2">
                <table className="min-w-full divide-y divide-gray-200 text-[13px] whitespace-nowrap">
                    <thead className="bg-gray-50">
                        <tr>
                            <th onClick={() => handleSort('id_product_satuan')} className="px-3 py-2 text-left font-bold text-gray-600 cursor-pointer group border-r border-gray-200 hover:bg-gray-100 transition-colors whitespace-nowrap w-1/4">
                                <div className="flex items-center justify-between">Kode Unit {getSortIcon('id_product_satuan')}</div>
                            </th>
                            <th onClick={() => handleSort('nm_product_satuan')} className="px-3 py-2 text-left font-bold text-gray-600 cursor-pointer group hover:bg-gray-100 transition-colors whitespace-nowrap">
                                <div className="flex items-center justify-between">Unit Name {getSortIcon('nm_product_satuan')}</div>
                            </th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-100">
                        {units.length === 0 ? (
                            ...
                            units.map((unit, index) => (
                                <tr key={unit.id_product_satuan} className={`border-b border-gray-100 hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                                    <td className="px-3 py-1.5 border-r border-gray-100 whitespace-nowrap">
                                        <Link to={`/productunit/${unit.id_product_satuan}/edit`} state={{ name: unit.nm_product_satuan }} className="text-gray-600 hover:text-[#3b82f6] font-medium block w-full">
                                            {unit.id_product_satuan}
                                        </Link>
                                    </td>
                                    <td className="px-3 py-1.5 whitespace-nowrap">
                                        <Link to={`/productunit/${unit.id_product_satuan}/edit`} state={{ name: unit.nm_product_satuan }} className="text-gray-600 hover:text-[#3b82f6] block w-full">
                                            {unit.nm_product_satuan}
                                        </Link>
                                    </td>
                                </tr>
                            ))
            ...
            <div className="flex justify-between items-center mt-3 pb-2 px-1 text-[13px] text-gray-600">
                <div>
                    Showing {((currentPage - 1) * perPage) + (units.length > 0 ? 1 : 0)} to {Math.min(currentPage * perPage, total)} of {total} entries
                </div>
