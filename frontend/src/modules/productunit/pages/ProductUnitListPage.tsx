...
const ProductUnitListPage: React.FC = () => {
    const [searchInput, setSearchInput] = useState<string>('');
    const { units, meta, loading, error, updateParams } = useProductUnit();

    const { data: menuInfo } = useQuery({
        queryKey: ["menu", "10504"],
        queryFn: async () => {
            const { data } = await api.get(`/menus/10504`);
            return data;
        },
    });
    // In legacy menu for product unit is 10504
    const menuTitle = menuInfo?.data?.nm_menu || menuInfo?.nm_menu || 'Product Unit';
...
export default ProductUnitListPage;
