...
const ProductCategoryTable: React.FC<ProductCategoryTableProps> = ({
    ...
    const total = meta?.total ? parseInt(meta.total.toString(), 10) : categories.length;
    ...
    return (
        <div className="w-full">
            ...
            <div className="overflow-x-auto border-t border-b border-gray-200 mt-2">
                <table className="min-w-full divide-y divide-gray-200 text-[13px] whitespace-nowrap">
                    <thead className="bg-gray-50">
                        <tr>
                            <th onClick={() => handleSort('kode_product_kategori')} className="px-3 py-2 text-left font-bold text-gray-600 cursor-pointer group border-r border-gray-200 hover:bg-gray-100 transition-colors whitespace-nowrap w-1/4">
                                <div className="flex items-center justify-between">Kode Category {getSortIcon('kode_product_kategori')}</div>
                            </th>
                            <th onClick={() => handleSort('nm_product_kategori')} className="px-3 py-2 text-left font-bold text-gray-600 cursor-pointer group hover:bg-gray-100 transition-colors whitespace-nowrap">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                        {categories.length === 0 ? (
                            <tr>
                                <td colSpan={2} className="px-3 py-10 text-center text-gray-500">Tidak ada data yang ditemukan.</td>
                            </tr>
                        ) : (
                            categories.map((cat, index) => (
                                <tr key={cat.id_product_kategori} className={`border-b border-gray-100 hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                                    <td className="px-3 py-1.5 border-r border-gray-100 whitespace-nowrap">
                                        <Link to={`/productcategory/${cat.id_product_kategori}/edit`} state={{ name: cat.nm_product_kategori }} className="text-gray-600 hover:text-[#3b82f6] font-medium block w-full">
                                            {cat.kode_product_kategori}
                                        </Link>
                                    </td>
                                    <td className="px-3 py-1.5 whitespace-nowrap">
                                        <Link to={`/productcategory/${cat.id_product_kategori}/edit`} state={{ name: cat.nm_product_kategori }} className="text-gray-600 hover:text-[#3b82f6] block w-full">
                                            {cat.nm_product_kategori}
                                        </Link>
                                    </td>
                                </tr>
                            ))
            ...
            <div className="flex justify-between items-center mt-3 pb-2 px-1 text-[13px] text-gray-600">
                <div>
                    Showing {((currentPage - 1) * perPage) + (categories.length > 0 ? 1 : 0)} to {Math.min(currentPage * perPage, total)} of {total} entries
                </div>
