...
import { getMenuInfo } from '../api/productCategoryApi';
import { useQuery } from '@tanstack/react-query';

const ProductCategoryListPage: React.FC = () => {
    const [searchInput, setSearchInput] = useState<string>('');
    const { categories, meta, loading, error, updateParams } = useProductCategory();

    const { data: menuInfo } = useQuery({
        queryKey: ["menu", "10502"],
        queryFn: () => getMenuInfo("10502"),
    });
    const menuTitle = menuInfo?.data?.nm_menu || menuInfo?.nm_menu || 'Product Category';
            ...
            <div className="bg-white rounded shadow-sm border border-gray-200">
                <div className="p-6">
                    <div className="flex justify-start mb-6">
                        ...
                        >
                            <span className="font-bold mr-1 text-base leading-none">+</span> Add Product Category
                        </Link>
                    </div>
