...
const CustomerListPage: React.FC = () => {
    ...
    const { customers, meta, loading, error, updateParams } = useCustomer();

    const { data: menuInfo } = useQuery({
        queryKey: ["menu", "10301"],
        queryFn: async () => {
            const { data } = await api.get(`/menus/10301`);
            return data;
        },
    });
    // Menu ID 10301
    const menuTitle = menuInfo?.data?.nm_menu || menuInfo?.nm_menu || 'Customer';
            ...
            <div className="bg-white rounded shadow-sm border border-gray-200">
                <div className="p-6">
                    <div className="flex justify-start mb-6">
                        ...
                        >
                            <span className="font-bold mr-1 text-base leading-none">+</span> Add Customer
                        </Link>
                    </div>
