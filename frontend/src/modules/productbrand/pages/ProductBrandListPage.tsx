...
import ProductBrandTable from '../components/ProductBrandTable';
import { useQuery } from '@tanstack/react-query';
import api from '../../../services/api';

const ProductBrandListPage: React.FC = () => {
    const [searchInput, setSearchInput] = useState<string>('');
    const { brands, meta, loading, error, updateParams } = useProductBrand();

    const { data: menuInfo } = useQuery({
        queryKey: ["menu", "10505"],
        queryFn: async () => {
            const { data } = await api.get(`/menus/10505`);
            return data;
        },
    });
    // Menu ID 10505
    const menuTitle = menuInfo?.data?.nm_menu || menuInfo?.nm_menu || 'Brand';
...
export default ProductBrandListPage;
