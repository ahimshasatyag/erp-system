...
const ProductBrandTable: React.FC<ProductBrandTableProps> = ({
    brands = [],
    meta = {},
    onParamsChange,
    searchElement
    ...
    const total = meta?.total ? parseInt(meta.total.toString(), 10) : brands.length;
    ...
    return (
        <div className="w-full">
            ...
            <div className="overflow-x-auto border-t border-b border-gray-200 mt-2">
                <table className="min-w-full divide-y divide-gray-200 text-[13px] whitespace-nowrap">
                    <thead className="bg-gray-50">
                        <tr>
                            <th onClick={() => handleSort('id_product_brand')} className="px-3 py-2 text-left font-bold text-gray-600 cursor-pointer group border-r border-gray-200 hover:bg-gray-100 transition-colors whitespace-nowrap w-1/4">
                                <div className="flex items-center justify-between">Kode Brand {getSortIcon('id_product_brand')}</div>
                            </th>
                            <th onClick={() => handleSort('nm_product_brand')} className="px-3 py-2 text-left font-bold text-gray-600 cursor-pointer group hover:bg-gray-100 transition-colors whitespace-nowrap">
                                <div className="flex items-center justify-between">Brand Name {getSortIcon('nm_product_brand')}</div>
                            </th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-100">
                        {brands.length === 0 ? (
                            ...
                            brands.map((brand, index) => (
                                <tr key={brand.id_product_brand} className={`border-b border-gray-100 hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                                    <td className="px-3 py-1.5 border-r border-gray-100 whitespace-nowrap">
                                        <Link to={`/productbrand/${brand.id_product_brand}/edit`} state={{ name: brand.nm_product_brand }} className="text-gray-600 hover:text-[#3b82f6] font-medium block w-full">
                                            {brand.id_product_brand}
                                        </Link>
                                    </td>
                                    <td className="px-3 py-1.5 whitespace-nowrap">
                                        <Link to={`/productbrand/${brand.id_product_brand}/edit`} state={{ name: brand.nm_product_brand }} className="text-gray-600 hover:text-[#3b82f6] block w-full">
            ...
            <div className="flex justify-between items-center mt-3 pb-2 px-1 text-[13px] text-gray-600">
                <div>
                    Showing {((currentPage - 1) * perPage) + (brands.length > 0 ? 1 : 0)} to {Math.min(currentPage * perPage, total)} of {total} entries
                </div>
