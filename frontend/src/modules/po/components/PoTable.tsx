...
const PoTable: React.FC<PoTableProps> = ({
    ...
    const total = meta?.total ? parseInt(meta.total.toString(), 10) : pos.length;
    ...
    return (
        <div className="w-full">
                    ...
                    <thead className="bg-gray-50">
                        <tr>
                            <th onClick={() => handleSort('code_po')} className="px-2 py-1.5 text-left font-bold text-gray-600 cursor-pointer hover:bg-gray-100 transition-colors">
                                <div className="flex items-center justify-between">PO Number {getSortIcon('code_po')}</div>
                            </th>
                            ...
                            pos.map((po, index) => (
                                <tr key={po.id_po} className={`border-b border-gray-100 hover:bg-gray-50 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                                    <td className="px-2 py-1 whitespace-nowrap">
                                        <Link to={`/po/edit/${po.id_po}`} state={{ name: po.code_po }} className="text-gray-900 hover:text-blue-600 block">
                                            {po.code_po}
                                        </Link>
                                    </td>
                                    <td className="px-2 py-1 whitespace-nowrap">
            ...
            <div className="flex justify-between items-center mt-3 pb-2 px-1 text-[13px] text-gray-600">
                <div>
                    Showing {((currentPage - 1) * perPage) + (pos.length > 0 ? 1 : 0)} to {Math.min(currentPage * perPage, total)} of {total} entries
                </div>
